function [stats] = evaluate_TVSum_video_new(videos_test, segVid, teInds)
addpath('./knapsack');
% teInds
...
segIdx = cell(1,n_videos);
for i=1:n_videos,
    shotBoundaries = segVid{i};
    
    segTmp = zeros(numel(shotBoundaries)-1,2);
    for j=1:numel(shotBoundaries)-1,
        segTmp(j,2) = shotBoundaries(j+1);
        segTmp(j,1) = shotBoundaries(j)+1;
    end
    segIdx{i} = segTmp;
end
